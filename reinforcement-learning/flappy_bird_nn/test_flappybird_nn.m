...
function test_nn(nn, trainTime, TrainEPISODES)

%% System Variables:
GameVer = '1.0.T.Q-NN';          % The first full playable game
...
total_score = 0;
%%%%%%%%%%%%%%
TEST.num_roll_outs = 20;
TEST.avg_score = 0.;
TEST.best = 0;


% Main Game
for episode=1:TEST.num_roll_outs
disp('----- BEGIN of NEW TESTING EPISODE ----- ');
initGame();
CurrentFrameNo = double(0);
collide = false;
...
%           Comment above dx and dy above and put in the "continuous version". 
%           What do you think the continuous versions should be?
            ...
            if gameover
                ...
                message = [ps, extractdata(gameover_qvalue(ps(3)))];
                display(message);
            end
            ps = [dx dy a];
            old_qvalues = qvalues;
    ...
    if frame_updated
        ...
        if ShowFPS
            ...
            set(var_text_handle, 'String', sprintf(['Testing EPISODES: %d\n'...
                                                    'Deaths: %d HighScore: %d\n' ...
                                                    'Avg Episode Score: %.2f, Total Score: %d\n'...
                                                    '--------------------------------------------------------\n'...
                                                    ['*The above testing results are trained with\n' ...
                                                    ' %.2f (s) and %d training episodes']
                                                    ], episode, deaths, TEST.best, TEST.avg_score, total_score, trainTime, TrainEPISODES));
        end
    end
    if fall_to_bottom
        if Score > TEST.best
    ...
    if (gameover)
        ...
        TEST.avg_score = (Score + TEST.avg_score * (episode - 1)) / episode;
