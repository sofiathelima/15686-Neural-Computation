...
nn.network = layerGraph([
   ...
   % you can also update the number
    fullyConnectedLayer(30, 'Name', 'fnn2' ),
    reluLayer('Name', 'relu2'),
%     fullyConnectedLayer(60, 'Name', 'fnn3' ),
%     reluLayer('Name', 'relu3'),
    fullyConnectedLayer(2, 'Name', 'fnn4' ),
]);
nn.network = dlnetwork(nn.network);

% Optimization
nn.lr = 0.01;
nn.avgGradientsNetwork = [];
nn.avgGradientsSquaredNetwork = [];
nn.iteration = 1;
nn.save_freq = 20;
...
% if (isfile("nn.mat"))
%     load("nn.mat");
%     nn.network = save_network;
%     nn.lr = save_lr;
%     nn.avgGradientsNetwork = save_avgGradientsNetwork;
%     nn.avgGradientsSquaredNetwork = save_avgGradientsSquaredNetwork;
%     nn.iteration = save_iteration;
% else
%     nn.network = layerGraph([
%         featureInputLayer(2, 'Name', 'Input'),
%         fullyConnectedLayer(15, 'Name', 'fnn1'),
...
run_flappybird_nn(nn, trainTime, EPISODES, deaths, highScore)

function run_flappybird_nn(nn, trainTime, EPISODES, deaths, highScore)
...
if ShowFPS
    fps_text_handle = text(10,20, 'FPS:60.0', 'Visible', 'off');
...
% Record Cache %      
%%%%%%%%%%%%%%%%
RL.record_n = 100;  % memory buffer size   -- Part 2a Task 2
RL.state_dim = 2;
RL.records = zeros(RL.state_dim + 1 + 1, RL.record_n); % (x, y, action, old_qvalue, updated_expected_reward)s
RL.pointer = 1;
RL.burnin = 0;
...
while 1
    ...
    while (curTime >= ((CurrentFrameNo) * GAME.FRAME_DURATION) && loops < GAME.MAX_FRAME_SKIP)
       ...
       if(toc(prevTime) >= 0.01 || gameover)
            ...
            agent_x = Bird.ScreenPos(1);
            agent_y = Bird.ScreenPos(2);
            GapY = [128 177] - (Tubes.VOffset(Tubes.FrontP)-1);

            tube_x = Tubes.ScreenX(Tubes.FrontP)+15;
            tube_y = GapY(2)-4 ;
            
            % distance from the ground
            % horizontal distance from tube lower-edge
            dx = max(min(floor((tube_x - agent_x)/4), 200) , 1);

            % vertical distance from tube right edge
            dy = max(min(floor((tube_y - agent_y)/4), 200) , 1);

%           For Part 2b -- turning into continuous from discrete state
%           Comment out  dx and dy above and put in the "continuous version" here. 
%           What do you think the continuous versions should be?
            ...
            if ps(1)~=-100
                if (~Flags.PreGame)
                    % only training samples will be recorded
                    [nn, RL] = update_records(nn, RL, ps, old_qvalues, dx, dy, reward, gameover);
                end
%                 % train
                if(toc(last_train_time) >= 100 || gameover)
                    %%%%%%%%%%%%%%%%%%%%
                    % train the network%
                    %%%%%%%%%%%%%%%%%%%%
                    last_train_time = tic;
                    if (RL.burnin == RL.record_n && Flags.train_flag)
                        [nn, RL] = train_q_network(nn, RL);
                    end
                end
            end
            ...
            if gameover
                ...
                gameover_qvalue = forward(nn.network, dlarray([ps(1); ps(2)], 'CB'));
                message = [ps, num2str(extractdata(gameover_qvalue(ps(3))))];
    ...
    if (gameover)
        ...
        save_network = nn.network;
        save_lr = nn.lr ;
        save_avgGradientsNetwork = nn.avgGradientsNetwork;
        save_avgGradientsSquaredNetwork = nn.avgGradientsSquaredNetwork;
        save_iteration = nn.iteration;

        
        save("nn.mat", 'save_network', 'save_lr', 'save_avgGradientsNetwork', ...
            'save_avgGradientsSquaredNetwork', 'save_iteration', ...
            'trainTime', 'EPISODES', 'deaths', 'highScore')
        EPISODES = EPISODES + 1;
