clear all;
%This code is for testing the Q table that is learned by running flappybird_qtable. You still need to fill in 
% Task 2 (dx, dy) to update the state, and Task 3 to fill out the conditional to decide whether to flap or not
% before you can run this code. 
% rng specify a particular sequence of pipes, so that everyone's Q table will be subject to the same test.

rng(1538615686);
load("Q.mat");
disp("Q table Loaded!...")

test_qtable(Q, trainTime, EPISODES)

function test_qtable(Q, trainTime, TrainEPISODES)

%% System Variables:
GameVer = '1.0.T.Qtable';          % The first full playable game
...
while 1
    ...
    while (curTime >= ((CurrentFrameNo) * GAME.FRAME_DURATION) && loops < GAME.MAX_FRAME_SKIP)
      ...
      %% Cycling the Palette
       ...
       if(toc(prevTime) >= 0.01 || gameover)
            ...
            dx = max(round((tube_x - agent_x)/4),1); % same as in training script
            % vertical distance from tube right edge
            dy = max(round((tube_y - agent_y)/4),1); % same as in training script
            ...
            if ((Q(dx,dy,2)>Q(dx,dy,1)) && ~gameover) % same as in training script
                a = 2;
                FlyKeyStatus = true;
            else 
                a = 1;
    ...
    if fall_to_bottom
        ...
        score_report = {sprintf('Score: %d', Score), sprintf('Test Best: %d', TEST.best)};
