...
%             disp(dy)
            ...
            if ~gameover
                ...
                % reward = 10 + 10*((GapY(1) < agent_y) && (GapY(2) > agent_y)) + 10*(GapY(1) < agent_y) + 10*(GapY(2) > agent_y);
                % fprintf('agent_y %d ; GapY(1) %d ; GapY(2) %d ; tube_y %d \n',agent_y,GapY(1),GapY(2),tube_y);
                % What areas are good for bird, e.g. bird not close to ground
                % 0 < agent_y means bird is below ceiling
            else
                reward = -1000;
