...
for stimulus=inputs
  ...
  freq = spikeFrequency(y_plot, 20, 5);
  freqs = [freqs freq];
%% Plotting results
% fig = figure;
%   plot(x_plot,y_plot); xlabel('Time'); ylabel('Voltage');
%   saveas(fig, sprintf('2b_I%s.fig', string(stimulus)));
end

%% Plotting FI curve

fig = figure;
plot(inputs, freqs); xlabel('Injected Current'); ylabel('Spike Frequency');
title('Problem 2 Part B: Numerical Integration of Spiking Neuron Model')
saveas(fig, '2b_FIcurve.fig')
saveas(fig, '2b_FIcurve.png')